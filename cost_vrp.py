...
def print_solution(data, manager, routing, assignment, dic):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        while not routing.IsEnd(index):
            ...
            print("....", route_distance, vehicle_id, routing.GetArcCostForVehicle(previous_index, index, vehicle_id))
...
def main(updated_data):
    ...
    def create_distance_callback1(data, v):
    ...
    def fix_callback(data,v):
        return data['Fixed_cost'][v]

    for vr in range(data['num_vehicles']):
        print(data["vehicle_VarCost"][vr])
        dist_callback1 = create_distance_callback1(data, data["vehicle_VarCost"][vr])
        evaluator_list.append(dist_callback1)
    for vehicle_id, evaluator in enumerate(evaluator_list):
        fix = routing.RegisterUnaryTransitCallback(fix_callback)
        print(",###",fix)
        temp_cost = int(routing.RegisterTransitCallback(evaluator))
